...
export default function Programcilar() {
  const [iyilerliste, setEnİyilerListe] = useState(enIyilerListesi);
  const [programcı, setProgramcı] = useState(null);
  ...
  const oneCikaninIsmi = () => {
    ...
    const secilenProgramci = iyilerliste.find((item) => {
      return item.id == programcı;
    });
  ...
  const stil = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: programcı == null ? "royalblue" : "gold", // 🤔 kutlarken renk gold'a dönecek
  };
